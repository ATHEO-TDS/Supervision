...
#
# DESCRIPTION:
# This PowerShell script is designed to monitor Veeam Backup & Replication (VBR) Replication job sessions.
...
# - Warning: At least one session is in a warning state or waiting for a retry.
...
try {
    ...
    $sessListRp = GetVBRBackupSession |  Where-Object { -not ($_.JobName -in $ExcludedJobsArray) } | Group-Object JobName | ForEach-Object { $_.Group | Sort-Object SessionEndTime -Descending | Select-Object -First 1}
