...
#
# DESCRIPTION:
# This PowerShell script is designed to monitor Veeam Backup & Replication (VBR) tape backup job sessions.
...
# - OK: All sessions are successful, idle or still running.
...
$sessListTp = @() # Array to store tape sessions
#endregion
...
try {
    ...
    if ($allJobsTp) {
        $sessListTp = $allJobsTp | ForEach-Object {[veeam.backup.core.cbackupsession]::GetByJob($_.Id)} | Where-Object { -not ($_.JobName -in $ExcludedJobsArray) } | Where-Object {
    ...
    if (-not $sessListTp) {
        Exit-Unknown "No tape session found"
    }
